...
export default function BookAppointmentForm({
  ...
  const getLocalISOString = (date: Date) => {
    // Format the date as ISO 8601 with local timezone offset.
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      const newAppointmentData = {
        doctor: session?.user?.id, // Replace with actual doctor ID if needed
    ...
    [verifiedPatient, appointmentData, router, dispatch, session?.user?.id]
  );
